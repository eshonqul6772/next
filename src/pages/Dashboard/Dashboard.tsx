...
const RECENT_ORDER_ITEMS = [1, 2, 3, 4, 5] as const;
const ORDER_NUMBER_BASE = 1000;
const ORDER_PRICE_BASE = 500;
const ORDER_PRICE_STEP = 100;
          ...
          <div className="space-y-3">
            {RECENT_ORDER_ITEMS.map(item => (
              <div key={item} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                <span className="text-sm text-gray-600">Order #{ORDER_NUMBER_BASE + item}</span>
                <span className="text-sm font-semibold text-gray-900">
                  ${ORDER_PRICE_BASE + item * ORDER_PRICE_STEP}
                </span>
              </div>
            ))}
          </div>
